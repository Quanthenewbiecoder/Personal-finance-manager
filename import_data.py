...
cursor.executemany(
    'INSERT OR IGNORE INTO users (username, email, password_hash) VALUES (?, ?, ?)', users)

# Insert fake transactions
transactions = [
    # (description, amount, date, category_id, user_id)
    ('Grocery shopping', 50.75, datetime.now(), 2, 1),
    ('Salary', 2000.00, datetime.now(), 1, 1),
    ('Coffee', 3.50, datetime.now(), 2, 2),
    ('Utilities', 120.00, datetime.now(), 2, 2)
]
cursor.executemany(
    'INSERT INTO transactions (description, amount, date, category_id, user_id) VALUES (?, ?, ?, ?, ?)', transactions)
